...
app.post("/register/", async (request, response) => {
  ...
  const getUser = `SELECT username FROM user 
  ...
  if (dbUser !== undefined) {
    ...
    if (password.length < 6) {
      ...
      const addQuery = `INSERT INTO user(username,password,name,gender)
          VALUES ('${username}','${hashedPassword}','${name}','${gender}');`;
      const addQueryDeatails = await database.run(addQuery);
      response.status(200);
...
app.post("/login/", async (request, response) => {
  ...
  const checkUser = `SELECT * FROM user
    WHERE username='${username}';`;
  const userExists = await database.get(checkUser);
